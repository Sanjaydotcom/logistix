...
export default {
  ...
  setup(props) {
    ...
    const handleFile = async (e) => {
      ...
      formData.append("basePath", "credentials");
      ...
      return {
        ...
        toDate: {
          ...
          mustBeCool: helpers.withMessage(
            "Issue Date must be less then Expiry Date",
            validateDateDiff(state.fromDate)
          ),
        },
      };
    });
  ...
  created() {
    this.UpdateFields();
    if (this.formType === "WPS") {
      this.IsWPS = true;
      this.state.fromDate="2022-06-02";
      this.state.toDate="2022-06-02";
    }
  },
  methods: {
    async submitForm() {
      ...
      if (!this.v$.$error && this.isFilePresent) {
        if (this.IsEdit) {
          ...
          await this.employeesStore.updateCredentials(
            { ...this.state, id: this.credentialId },
            this.id,
            this.credentialId
          );
        } else {
          ...
          this.state["fileId"] = fileId;
          this.state["fileName"] = this.fileName;
          await this.employeesStore.addCredentials(this.state, this.id);
        }
    ...
    async UpdateFields() {
      if (this.IsEdit) {
        this.titleText = "Edit";
        this.DataById = await this.employeesStore.getCredentialsByCredentialId(
          this.id,
          this.credentialId
        );
        this.state.number = this.DataById.number;
        this.state.fileId = this.DataById.fileId;
        this.state.fileName = this.DataById.fileName;
        this.state.fromDate = this.DataById.fromDate;
        this.state.toDate = this.DataById.toDate;
