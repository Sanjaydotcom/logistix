...
export default {
  ...
  setup() {
    ...
    const filterList = (search) => {
      state.data = store.getDirectoryLists().filter((data) => {
...
<template>
  <div class="sm:hidden xl:order-2 xl:block xl:flex-shrink-0">
    <div
      class="bg-themeBackground relative flex min-h-screen w-96 flex-col border-r border-gray-200"
    >
      ...
      <nav class="min-h-0 flex-1 overflow-y-auto" aria-label="Directory">
        <ul role="list" class="relative z-0 divide-y divide-gray-200">
          ...
          <li v-for="data in state.data" :key="data.id">
            <NuxtLink :to="`/app/crm/customer/${data.id}`">
              <div
                class="relative flex space-x-3 rounded-md px-6 py-3"
                :class="[
                  $route.path == `/customer/${data.id}`
              ...
              >
                <img
                  ...
                  :src="[
                    data.details?.general?.profilePicFileId
                      ? `https://api.logistix.dev/document/document/documents/show/${data.details.general.profilePicFileId}`
