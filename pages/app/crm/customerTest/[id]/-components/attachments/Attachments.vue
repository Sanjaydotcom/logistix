...
export default {
  ...
  setup(props) {
    ...
    const deleteAttachment = async () => {
      ...
      await store.deleteAttachments(props.id, selectedAttachmentId.value);
    };
    watch(useCustomerStore(), () => {
      fetchAttachments();
    });
    const fetchAttachments = async () => {
      // await store.getEmployeeById(props.id);
      data.value = await store.getAttachmentsDetails();
    };
