...
export default {
  ...
  setup() {
    ...
    const { codesUpdated } = storeToRefs(store);
    ...
    const deleteList = async () => {
      ...
      router.push("/app/shipping/masters/codes/");
    };
    watch(codesUpdated, () => {
      if (codesUpdated.value) {
    ...
    const showNextItem = () => {
      router.push(`/app/shipping/masters/codes/${Number(state.id) + 1}`);
    };
    const showPrevItem = () => {
      router.push(`/app/shipping/masters/codes/${Number(state.id) - 1}`);
    };
    ...
    >
      <div class="flex h-full items-center justify-center">
        ...
        <div class="text-center">
          ...
          <p class="mt-5 text-sm text-gray-500">Select a code</p>
        </div>
      </div>
    </section>
    <!-- Message list-->
