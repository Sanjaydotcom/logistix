...
export default {
  ...
  setup(props) {
    ...
    onMounted(() => {
      if (props.id && props.editData) {
        state.termValue = props.editData.termValue;
        state.displayOrder = props.editData.displayOrder;
  ...
  methods: {
    async submitForm() {
      ...
      if (!this.v$.$error) {
        if (this.id) {
          await this.store.editTermItem(this.state, this.id);
        } else {
                  ...
                  <div class="divide-y divide-gray-200 px-4 sm:px-6">
                    <div class="space-y-6 pt-6 pb-5">
                      <div>
                        ...
                        <span
                          v-if="v$.termValue.$error"
                          class="font-small text-xs text-red-600"
                        >
                          {{ v$.termValue.$errors[0].$message }}
                        </span>
                      </div>
                      <div>
                        <serviceTypeDropDown
