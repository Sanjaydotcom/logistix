<script>
import { useFetch } from "@vueuse/core";
import { URL } from "@/composables/api/index";
import { useEmployeesStore } from "~~/store/employees";
export default {
  props: ["id"],
  setup(props) {
    const fileId = ref(null);
    const employeesStore = useEmployeesStore();
    const emptyImg =
      "data:image/png;base64,";
    const profileImageIsHovering = ref(false);

    const profilePicMouseHover = async () => {
      profileImageIsHovering.value = true;
    };

    const profilePicMouseOut = async () => {
      profileImageIsHovering.value = false;
    };

    const uploadFile = async (data) => {
      try {
        const fileUpload = await useFetch(`${URL.Document.UPLOAD}`)
          .post(data)
          .json();
        return fileUpload.data.value;
      } catch (err) {
        console.log(err);
      }
    };

    const updateProfile = async (data) => {
      const profileImageId = await uploadFile(data);
      await employeesStore.updateProfileImage(profileImageId, props.id);
      if (profileImageId) {
        fileId.value = profileImageId;
      }
    };

    const handleImage = async (e) => {
      const file = e.target.files[0];
      const formData = new FormData();
      formData.append("inputStream", file);
      formData.append("fileName", file.name);
      formData.append("basePath", "profile");
      formData.append("contentType", "image");
      formData.append("documentType", "image");
      updateProfile(formData);
    };

    const fetchUserProfile = async () => {
      const data = await employeesStore.getEmployeeById(props.id);
      fileId.value = data.details?.profilePicFileId;
    };

    onMounted(() => fetchUserProfile());

    return {
      profilePicMouseHover,
      profilePicMouseOut,
      profileImageIsHovering,
      handleImage,
      URL,
      fileId,
      emptyImg,
    };
  },
};
</script>
<template>
  <div class="flex">
    <label for="imageUpload">
      <img
        :class="[
          profileImageIsHovering ? 'cursor-pointer opacity-50' : 'opacity-100',
          'h-24, w-24 rounded-full object-cover ring-4 ring-white sm:h-32 sm:w-32',
        ]"
        :src="[
          fileId
            ? `https://api.logistix.dev/document/document/documents/show/${fileId}`
            : emptyImg,
        ]"
        @mouseover="profilePicMouseHover"
        @mouseout="profilePicMouseOut"
        alt=""
      />
    </label>
    <input
      id="imageUpload"
      class="hidden"
      type="file"
      accept="image/jpeg,image/jpg,image/png"
      @change="handleImage"
    />
  </div>
</template>
